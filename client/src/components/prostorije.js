...
const Prostorije = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/info/upis-prostorije', prostorija);
  ...
  const handleDelete = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/info/brisanje-prostorije', prostorija);
      const message = response.data.message;
